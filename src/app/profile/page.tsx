...
export default function ProfilePage() {
    ...
    async function uploadImage(event: any) {
        ...
        try {
            ...
            setImageData(prevImageData => [...prevImageData, response?.data?.image]);
            setLoading(false)
        } catch (error) {
            console.log(error);
        }
        ...
        <div className="w-full h-screen">
            <div className="w-full items-center bg-black h-96 flex flex-col">
                <div className="bg-gray-400 mt-10 p-10 rounded-sm">
                    <h1 className="text-center text-black font-bold text-xl">{loading ? "Uploading...." : "Upload Image"}</h1>

                    <form onSubmit={uploadImage}>
                        <label className="block my-2 ps-2" htmlFor="image">Image</label>
                        <input
                            id="image"
                            type="file"
                        ...
                        />
                        <button
                            className={`block ${loading ? " cursor-not-allowed" : ""} my-2 p-2 bg-blue-600 rounded-sm`}
            ...
            <div
                className="w-full gap-5 flex flex-wrap h-fit "
            >
                {
                    imageData.map((item, index) => (
                        <img
                            className="h-40 w-40 object-cover"
                            src={item?.imageUrl} alt="index" />
